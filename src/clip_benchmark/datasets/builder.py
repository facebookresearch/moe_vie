...
from . import (flickr, imagenetv2, objectnet, video_classification_dataset, video_retrieval_dataset)


_DATA_ROOT = os.environ.get("CLIP_BENCHMARK_DATA_ROOT", "datasets")
_UCF101_LABELS = os.environ.get("UCF101_LABEL_MAP", "<MAP PATH>")
_HMDB_LABELS = os.environ.get("HMDB_LABEL_MAP", "<MAP PATH>")


def build_dataset(dataset_name, root="root", transform=None, split="test", download=True, annotation_file=None, language="en", task="zeroshot_classification", wds_cache_dir=None, custom_classname_file=None, custom_template_file=None, num_frames=8, **kwargs):
    ...
    if dataset_name in video_classification_datasets.keys():
        ...
        ds = video_classification_dataset.VideoClassificationDataset("", task_config, transform, num_frames=num_frames)
    elif dataset_name == "msrvtt":
        ds = video_retrieval_dataset.VideoRetrievalDataset(os.path.join(_DATA_ROOT, "MSRVTT/msrvtt_data/MSRVTT_JSFUSION_test.csv"), os.path.join(_DATA_ROOT, "MSRVTT/videos/all/"), transform, num_frames=num_frames)
        ...
        ds = pos_neg_caption_dataset.PosNegCaptionDataset(root=images_dir, ann_file=annotation_file, transform=transform, crop_images=True, **kwargs)
        ...
        root_split = os.path.join(dataset_dir, archive_name.replace(".zip", ""))
        if not os.path.exists(root_split):
            print(f"Downloading coco captions {archive_name}...")
            if not os.path.exists(os.path.join(dataset_dir, archive_name)):
                call(f"wget http://images.cocodataset.org/zips/{archive_name} --output-document={dataset_dir}/{archive_name}", shell=True)
            call(f"unzip {dataset_dir}/{archive_name} -d {dataset_dir}", shell=True)
        ...
        if not os.path.exists(annotation_file):
            ...
            call(f"wget {url} --output-document={annotation_file}", shell=True)
        ds = pos_neg_caption_dataset.PosNegCaptionDataset(root=images_dir, ann_file=annotation_file, transform=transform, **kwargs)
...
def get_dataset_default_task(dataset):
    ...
    if dataset in ("flickr30k", "flickr8k", "mscoco_captions", "multilingual_mscoco_captions", "flickr30k-200", "crossmodal3600", "xtd200", "flickr8k_ocr", "rendered_ocr", "flickr30k_ocr", "mscoco_ocr", "text_cap", "pug_spar", "msrvtt"):
        return "zeroshot_retrieval"
    elif dataset in ("pug_animals"):
        return "multiclass_retrieval"
    elif dataset.startswith("sugar_crepe") or dataset == "winoground" or dataset == "aro_visual_attribution" or dataset.startswith("pug_animals"):
        return "image_caption_selection"
    else:
        return "zeroshot_classification"

def is_video_dataset(dataset):
    if dataset in ("k400_val", "k600_val", "k700_val", "ucf101_val", "hmdb_test", "mitv1_val", "ssv2_mc_val", "charades_mc_test", "msrvtt"):
        return True
    else:
        return False
...
def get_dataset_collate_fn(dataset_name):
    ...
    if dataset_name in ("mscoco_captions", "multilingual_mscoco_captions", "flickr30k", "flickr8k", "flickr30k-200", "crossmodal3600", "xtd200", "winoground", "flickr8k_ocr", "rendered_ocr", "flickr30k_ocr", "mscoco_ocr", "aro_visual_attribution", "text_cap", "pug_spar", "msrvtt") or dataset_name.startswith("sugar_crepe") or dataset_name.startswith("pug_animals"):
...
def build_vtab_dataset(dataset_name, transform, download=True, split="test", data_dir="root", classnames=[]):
    ...
    if dataset_name == "caltech101":
        ...
        download_tfds_dataset("dmlab", data_dir=data_dir)
...
def build_wds_dataset(dataset_name, transform, split="test", data_dir="root", cache_dir=None):
    ...
    if dataset_type == "retrieval":
        ...
        dataset.retrieval_template = json.load(open(os.path.join(metadata_dir, "retrieval_template.json")))
    else:
        label_type = "npy" if dataset_type == "multilabel" else "cls"
        dataset = (dataset
...
video_classification_datasets = {
    "k400_val": {
        "media": os.path.join(_DATA_ROOT, "kinetics/k400/videos/val"),
        "labels": None,
        "media_type": "video",
        "templates": None,
    },
    "k600_val": {
        "media": os.path.join(_DATA_ROOT, "kinetics/k600/videos/val"),
        "labels": None,
        "media_type": "video",
        "templates": None,
    },
    "k700_val": {
        "media": os.path.join(_DATA_ROOT, "kinetics/k700-2020/val"),
        "labels": None,
        "media_type": "video",
        "templates": None,
    },
    "ucf101_val": {
        "media": os.path.join(_DATA_ROOT, "ucf101/UCF-101"),
        "labels": _UCF101_LABELS,
        "media_type": "video",
        "templates": None,
    },
    "hmdb_test": {
        "media": os.path.join(_DATA_ROOT, "hmdb/hmdb51"),
        "labels": _HMDB_LABELS,
        "media_type": "video",
        "templates": None,
    },
    "mitv1_val": {
        "media": os.path.join(_DATA_ROOT, "moments_in_time/validation"),
        "labels": None,
        "media_type": "video",
        "templates": None,
    },
    "ssv2_mc_val": {
        "media": os.path.join(_DATA_ROOT, "ssv2/validation"),
        "labels": None,
        "media_type": "video",
        "templates": None,
    },
